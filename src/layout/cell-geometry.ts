...
export function geometryForCell(slot: CellSlot, item: CanvasOccupancy | undefined): CellGeometry {
  ...
  if (item.kind === "question") return { footprint: "shell", scale: 1.22, aspectRatio: 1.05 };
  if (item.kind === "suggestion") {
    if (item.optionIndex === 0) return { footprint: "capsule", scale: 0.94, aspectRatio: 1.18 };
    if (item.optionIndex === 1) return { footprint: "orb", scale: 0.98, aspectRatio: 1 };
    return { footprint: "pebble", scale: 0.94, aspectRatio: 0.9 };
  }
  if (item.kind === "answer") return { footprint: "capsule", scale: 0.98, aspectRatio: 1.1 };
